{ pkgs, lib, config, hostsPath, host, ... }:
let
  modifier = "SUPER";
  inherit (import "${hostsPath}/${host}/variables.nix") terminal browser;
in {

  options = { binds_module.enable = lib.mkEnableOption "enables binds"; };

  config = lib.mkIf config.binds_module.enable {
